...
def _require_env(
    ...
    """
    Return an environment value or raise
    a readable setup error.
    """
...
async def _get_server_tool(
    ...
    server from crashing an unrelated Tavily request.
    """
    ...
    if server_name == "tavily":
        ...
        if shutil.which("uvx") is None:
            ...
            raise RuntimeError(
                "uvx was not found. Install uv, "
                "reopen the terminal, activate the "
                "travel environment, and run "
                "`uvx --version`."
            )
...
async def get_all_tools() -> None:
    ...
    One failed server will not stop
    the remaining tests.
    """
